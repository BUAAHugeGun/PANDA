...
class PANDA_dataset(Dataset):
    def __init__(self, path: str, split: str, valid_num, valid_blocks):
        ...
        if split == "train" or split == "valid":
            with open(os.path.join(path, "train_small.csv"), 'r') as f:
                ...
                for row in csv_f:
                    self.data.append([row['image_id'], row['data_provider'], int(row['isup_grade']), row['gleason_score']])
            self.data_check()
            self.data.sort()
    ...
    def data_check(self):
        ...
        for x in self.data:
            name = x[0]
            if not os.path.exists(os.path.join(self.path, IMAGE_DIR, name + ".png")):
                missing.append(name)
        if len(missing):
    ...
    def __getitem__(self, x):
        x = self.data[x]
        img = cv2.imread(os.path.join(self.path, IMAGE_DIR, x[0] + ".png"))
...
def build_data(path: str, batch_size, train: str, num_worker, valid_num=200, valid_block=25):
